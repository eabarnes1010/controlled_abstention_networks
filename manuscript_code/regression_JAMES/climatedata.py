"""Load and process synthetic climate data for abstention loss.
...
__date__ = "March 22 2021"

#----------------------------------------------------------------    
def get_climate_data(EXPINFO, to_plot=False):
    ...
    if 'SSTrand' not in globals():
        ...
        if(SIMPLE_DATA==True):
            SSTrand, y, lat, lon = load_simpledata(size='15x60')
        elif(SIMPLE_DATA==False):
            SSTrand, y, lat, lon = load_data()
        else:
            SSTrand, y, lat, lon = load_simpledata(size=SIMPLE_DATA)

    lat = np.squeeze(lat)
    lon = np.squeeze(lon)
    print('SST shape = ' + str(np.shape(SSTrand)))

    # make y values standardized for regression setup
    y = (y - np.mean(y))
    y = y/np.std(y)
    ...
    DATA_NAME = EXPINFO['exp_name']

    #----------------------------
    X, y_cat, tranquil, corrupt = add_reg_noise(data_name=DATA_NAME, 
                                                       X=SSTrand[:NSAMPLES], 
                                                       y=y[:NSAMPLES], 
                                                       lat=lat, 
                                                       lon=lon, 
    ...
    data_train, data_val, data_test = split_data(X, y_cat, tranquil, corrupt)
    X_train, y_train, tr_train, cr_train = data_train
    X_val, y_val, tr_val, cr_val = data_val
    X_test, y_test, tr_test, cr_test = data_test
    
    if(EXPINFO['boxcox']==True):
        print('\ntransforming the data with stats.boxcox...')
        shift = np.min(y_train[:,0])*1.25
        y_train,lam = stats.boxcox(y_train[:,0]-shift,None)
        y_train = y_train[:,np.newaxis]
        y_val = stats.boxcox(y_val[:,0]-shift,lam)[:,np.newaxis]
        y_test = stats.boxcox(y_test[:,0]-shift,lam)[:,np.newaxis]
        print('     lam = ' + str(lam) + '\n')
    ...
    X_train_std, onehot, X_val_std, onehot_val, xmean, xstd = preprocess_reg_data(X_train, y_train, X_val, y_val, NLABEL)
    __, __, X_test_std, onehot_test, __, __ = preprocess_reg_data(X_train, y_train, X_test, y_test, NLABEL)    
    ...
    if to_plot:
        ...
        plt.subplot(2,2,4)
        plt.hist(y_train[cr_train==1],20)
        plt.xlabel('class')
        plt.title('corrupted labels')

        plt.subplot(2,2,3)
        plt.hist(y_train[tr_train==1],20)
        plt.xlabel('class')
        plt.title('tranquil labels')

        plt.subplot(2,2,2)
        plt.hist(y_train[tr_train==0],20)
        plt.xlabel('class')
    ...
    return X_train_std, onehot, X_val_std, onehot_val, X_test_std, onehot_test, xmean, xstd, tr_train, tr_val, tr_test
...
def preprocess_reg_data(X_train, y_train, X_val, y_val, ABSTAIN):
    ...
    onehotlabels = np.append(y_train,np.zeros(shape=(np.shape(y_train)[0],ABSTAIN-1)),axis=1)
    onehotlabels_val = np.append(y_val,np.zeros(shape=(np.shape(y_val)[0],ABSTAIN-1)),axis=1)    
...
def add_reg_noise(data_name, X, y, lat, lon, pr_noise, nlabel, y_perc = np.nan, cutoff = 1., region_name='ENSO'):
    ...
    elif data_name.find('corruptInputs') == 0:      
        inoise = np.random.choice(np.arange(0,NSAMPLES),
                                  size=int(np.round(pr_noise*NSAMPLES)),
                                  replace=False,)
        for i in inoise:   
            igrid = np.random.choice(np.arange(len(lon)*len(lat)),
                                     size = 600,#180,#600,
                                     replace = False,
                                    )
            Xsample = X[i,:,:].flatten()
            Xsample[igrid] = -4.  # changed from 0
            X[i,:,:] = np.copy(np.reshape(Xsample,newshape=(len(lat),len(lon))))
            
        tranquil = is_tranquil(NSAMPLES,inoise)
        pr_noise = 0.0
        
#         plt.figure(figsize=(12*.75,2.73*2*.75))
#         iplot = 11
#         mapProj = ct.crs.EqualEarth(central_longitude = 0.)
#         ax = plt.subplot(1,2,1,projection=mapProj)
#         xplot = X[iplot,:,:]
...
#                                 extent='both',
#                                )
#         plt.text(0,1,'(a)',transform=ax.transAxes,fontsize=16)
...
#                                 extent='both',
#                                )
#         plt.text(0,1,'(b)',transform=ax.transAxes,fontsize=16)
#         plt.tight_layout()
#         plt.savefig('figures/final_figures/' + data_name + '_corruptmap.png', dpi=300)        
#         plt.show()         
